...
/**
 * 解析常规Json串,嵌套Json串,嵌套Json组
 * */
public class Json操作 {
    /**
     * JSON实际上也是键值对（"key":"value"）,Json可以嵌套,Map不能嵌套
    ...
    private static void strWritedToJSONObject() {
        ...
        String myJsonObj = "{\n" +
                ...
                "    }\n" +
                "}";
        JSONObject jsonobj= JSON.parseObject(myJsonObj);//将json字符串转换成Object对象
    ...
    public static void javaBeanToJsonStr(){
        Area bean1=new Area("上海浦东","上海虹口");
        JsonBaseBean message=new JsonBaseBean("zhangsan","男",bean1);
        String jsonStr= JSON.toJSONString(message);
        System.out.println("javaBean转化为Jsonstr为:"+jsonStr);
    }
    public static void main(String[] args) {
//        strWritedToJSONObject();
//        ParseJsonArray(); //解析json嵌套数组
        javaBeanToJsonStr();
    }
...
class JsonBaseBean{
    ...
    Area area;
    public JsonBaseBean(){}
    public JsonBaseBean(String name,String sex,Area area){
        this.name=name;
        this.sex=sex;
        this.area=area;
    }
...
class Area{
    ...
    String houseArea;
    public Area(){}
    public Area(String workArea,String houseArea){
